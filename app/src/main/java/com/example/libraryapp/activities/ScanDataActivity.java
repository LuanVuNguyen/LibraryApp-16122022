...
public class ScanDataActivity extends AppCompatActivity implements View.OnClickListener, HttpRfidResponse {
    ...
    private void initDeviceScanVN(){
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                connectThreadScan.connect(bluetoothDeviceConnected2(), ScanDataActivity.this, new Callable() {
                    @Override
                    public void call(boolean result) {
                        showToast("起動中…");
                        dismissProgress();
    ...
    private void initViews(){
        ...
        btn_startscan=(ImageView) findViewById(R.id.btn_startscan);
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                ...
                switch (id)
                {
                    case R.id.nav_account:
                        Toast.makeText(ScanDataActivity.this,"近日公開!",Toast.LENGTH_SHORT).show();break;
...
/*            case R.id.btn_search:
    ...
    private void eventExportYes(){
        ...
        if(!db.getAllProductsinvbyType("inventory").isEmpty()){
            showProgressRunUi();
            String[] header = new String[] {"RFID", "BARCODE", "NAME PRODUCT","QUANTITY"};
...
//        }
    ...
    public void progressRfidFinish(String output, int typeRequestApi, String fileName) {
        // KILL ALL HTTP
        System.out.println("TestOUTPUTSCAN  "+output);
        if(output.contains("Exception")){
            for(HttpPostRfid http : listHttp){
                http.cancel(true);
            }
        }
        System.out.println("NGUYENVULUAN "+output);
